...
def get_rate_sellers(json_file, driver):
  ...
  for id in data_list:
    ...
    time.sleep(1)
    try:
      rating = driver.find_element(By.CLASS_NAME, "address-rate-mini").text
    except Exception:
...
def get_content_root(url):
  ...
  for page in range(1, 101):
      ...
      for data in json_file['data']['products']:
        ...
        list_price.append({'idShop': data['supplierId'],
...
def main():
  ...
  for row in brands.itertuples(index=False):
    ...
    # print(makeDateList()) 
    # print(get_count_review_for_week(get_content_root(url_reviews), makeDateList()))
    # print(get_content_root(url_reviews))
    # print(row[0])
    # print(url_sellers)
    # print(url_cards)
    # print(get_count_cards(get_content(url_cards)))
    # print(get_count_sellers(get_content(url_sellers)))
    # print(get_rate_sellers(get_content(url_sellers), driver))
